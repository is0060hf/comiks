<?php
	...
	// 引数から初期ページを取得する
	$pageName = TableRegistry::get('PageNames')->find('All')->where(['id' => $categoryName->page_name_id])->first();
	$user_id = null;
	$pageList = null;

	if ($pageName) {
		$user = TableRegistry::get('Users')->find('All')->where(['id' => $pageName->user_id])->first();

		if ($user) {
			$user_id = $user->id;
			$pageList = TableRegistry::get('PageNames')->find('All')->where(['user_id' => $user->id]);
		} else {
...
<div class="breadcrumb_div">
	<ol class="breadcrumb m-b-20">
		...
		<li class="breadcrumb-item"><a href="<?php echo $this->Url->build(['controller' => 'CategoryNames', 'action' => 'index']); ?>">カテゴリ一覧</a></li>
		<li class="breadcrumb-item active">新規カテゴリ</li>
	</ol>
</div>
...
<div class="posts form large-9 medium-8 columns content">
	...
	<div class="row mt-4">
		<div class="col-12 text-center">
			...
			<a href="<?= $this->Url->build(['controller' => 'PageNames', 'action' => 'view', $categoryName->page_name_id]); ?>" class="btn btn-primary">
				<i class="fe-skip-back"></i>
				<span>固定ページ詳細に戻る</span>
			</a>
		</div>
	</div>
	<?= $this->Form->end() ?>
