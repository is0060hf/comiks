window.onload = function(){
	...
	$(function () {
		...
		drFeatured.on('dropify.afterClear', function(event, element){
			...
			form_data.append('category_id', $('#view_category_id').val());

			console.log($('#view_category_id').val());
			$.ajax({
				data: form_data,
				type: 'POST',
				dataType: 'json',
				url: '/categories/ajaxDeleteFeaturedImagePath',
				cache: false,
				contentType: false,
				processData: false,
	...
	$(document).on('change', '#default_featured_image_path', function() {
		...
		$.ajax({
			...
			type: 'POST',
			dataType: 'json',
			url: '/categories/ajaxUpdateFeaturedImagePath',
			cache: false,
			contentType: false,
			processData: false,
			beforeSend: function (xhr) {
